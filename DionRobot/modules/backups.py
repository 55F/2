...
import DionRobot.modules.sql.notes_sql as sql
from DionRobot import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from DionRobot.__main__ import DATA_IMPORT
from DionRobot.modules.helper_funcs.chat_status import user_admin
from DionRobot.modules.helper_funcs.alternate import typing_action

from DionRobot.modules.rules import get_rules
import DionRobot.modules.sql.rules_sql as rulessql

from DionRobot.modules.sql import warns_sql as warnssql
import DionRobot.modules.sql.blacklist_sql as blacklistsql
from DionRobot.modules.sql import disable_sql as disabledsql

from DionRobot.modules.sql import cust_filters_sql as filtersql
import DionRobot.modules.sql.welcome_sql as welcsql
import DionRobot.modules.sql.locks_sql as locksql
from DionRobot.modules.connection import connected
...
def export_data(update, context):
    ...
    f = open("DionRobot{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("DionRobot{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Dion-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("DionRobot{}.backup".format(chat_id))  # Cleaning file
