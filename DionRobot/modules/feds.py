...
from DionRobot.modules.helper_funcs.chat_status import is_user_admin
from DionRobot.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
from DionRobot.modules.helper_funcs.string_handling import markdown_parser
from DionRobot.modules.disable import DisableAbleCommandHandler

import DionRobot.modules.sql.feds_sql as sql

from DionRobot.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "DionRobot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="DionRobot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "DionRobot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="DionRobot_fbanned_users.csv",
